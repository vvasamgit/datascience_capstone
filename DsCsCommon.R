...
capstoneFileSuffix<-"capstone"

corpdir<-"corpus"
corpFilePefix<-"corp_"

tokenDir<-"tokens"
tokenFilePrefix<-"tkn_"

nGramsDir<-"ngrams"
allTokenFilePrefix<-"alltkn_file"
nGramFilePrefix<-"nGram_"
dfmFilePrefix<-"dfm_"
...
tkndirExists<-ifelse(dir.exists(file.path(dataDirHome, tokenDir)), TRUE, dir.create(file.path(dataDirHome, tokenDir)))

nGramsDirExists<-ifelse(dir.exists(file.path(dataDirHome, nGramsDir)), TRUE, dir.create(file.path(dataDirHome, nGramsDir)))

mwmDirExists<-ifelse(dir.exists(file.path(dataDirHome, nwmDir)), TRUE, dir.create(file.path(dataDirHome, nwmDir)))

corpDirHome<-paste0(dataDirHome,.Platform$file.sep,corpdir)

corpFilePattern<-paste0(corpFilePefix,"(.)*.rds")

tknDirHome<-paste0(dataDirHome,.Platform$file.sep,tokenDir)

tknFilePattern<-paste0(tokenFilePrefix,"(.)*.rds")

ngramDirHome<-paste0(dataDirHome,.Platform$file.sep,nGramsDir)

alltknFilePattern<-paste0(allTokenFilePrefix,"(.)*.rds")

ngramFilePattern<-paste0(nGramFilePrefix,"(.)*.rds")

nwmDirHome<-paste0(dataDirHome,.Platform$file.sep,nwmDir)

nwmFilePattern<-paste0(nwmFilePrefix,"(.)*.rds")
...
createNgram<-function(pCorpusTokens,nGramCnt) {
  
  ngramToken <- tokens_ngrams(pCorpusTokens,nGramCnt)
  return(ngramToken)
}

createDfmRemoveStopwords<-function(pNgramToken) {
  
  docDfm <- dfm(pNgramToken,tolower = TRUE, remove=stopwords('en'))
